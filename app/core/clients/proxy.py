...
# 404/405/426 mean the endpoint does not support the requested upgrade;
# upstream 5xx responses are transport-side failures before any response event
# and are safe to retry through the equivalent HTTP stream.  Auth (401/403),
# rate-limit/quota (409/429), and validation (400) remain visible so fallback
# cannot hide account or request semantics.
_AUTO_WEBSOCKET_HANDSHAKE_FALLBACK_STATUSES = frozenset({404, 405, 426})
...
_DEFER_WEBSOCKET_CONNECT_FAILURE_HEALTH: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "defer_websocket_connect_failure_health",
    default=False,
)
...
def _should_fallback_to_http_after_websocket_transport_error(
    ...
    *,
    ...
    """Return whether an unproductive WebSocket attempt may be replayed over HTTP.

    Reissuing a request after a downstream event has escaped can duplicate visible
    text or tool calls. The fallback is therefore intentionally limited to the
    connection/first-event phase. Structured upstream HTTP failures (notably
    auth, rate-limit and quota responses) keep their original semantics.
    """

    if transport_mode != "auto" or received_event:
        return False
    if isinstance(exc, CodexTransportError):
        if exc.status_code is not None:
            return _should_fallback_to_http_after_websocket_status(transport_mode, exc.status_code)
        return bool(exc.retryable_same_contract or exc.error_code == PROCESS_NETWORK_UNAVAILABLE_CODE)
    return isinstance(
        exc,
        (
...
async def _open_upstream_websocket(
    *,
    ...
    record_connect_failure = not _DEFER_WEBSOCKET_CONNECT_FAILURE_HEALTH.get()
    ...
    if not callable(request_obj):
        try:
            ...
            if circuit_breaker is not None and record_connect_failure:
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                and record_connect_failure
                and not _cb_recorded
...
async def _stream_responses_via_websocket(
    *,
    ...
    enforce_openai_sdk_contract: bool = True,
    defer_pre_event_failure_health: bool = False,
) -> AsyncIterator[tuple[str, str | None]]:
    ...
    async def _record_lifecycle_failure(exc: Exception) -> None:
        ...
        if circuit_breaker is None or lifecycle_recorded:
            return
        if defer_pre_event_failure_health and not seen_event:
            return
        await _record_account_circuit_breaker_failure(circuit_breaker, exc)
        lifecycle_recorded = True
    ...
    if route is not None:
        ...
        health_token = _DEFER_WEBSOCKET_CONNECT_FAILURE_HEALTH.set(defer_pre_event_failure_health)
        try:
            try:
                ...
                _DEFER_WEBSOCKET_CONNECT_FAILURE_HEALTH.reset(health_token)
        except Exception as exc:
            error_code = process_network_error_code(
                exc,
                fallback="upstream_unavailable",
    ...
    try:
        ...
        if not seen_terminal:
            premature_close = aiohttp.ClientError("Upstream websocket closed without terminal event")
            await _record_lifecycle_failure(premature_close)
            if defer_pre_event_failure_health and not seen_event:
                raise CodexTransportError(
                    "Upstream websocket closed before its first response event",
                    error_code=PROCESS_NETWORK_UNAVAILABLE_CODE,
                    retryable_same_contract=True,
                )
    except Exception as exc:
        await _record_lifecycle_failure(exc)
...
async def _stream_responses_with_session(
    ...
    async def _stream_via_http_after_websocket_failure(
        *,
        ...
        logger.warning(
            "upstream_websocket_unavailable request_id=%s status=%s target=%s retrying_transport=http",
            get_request_id(),
            rejection_status,
            _summarize_upstream_target(url),
        )
        _maybe_log_upstream_request_complete(
            ...
            error_code="upstream_websocket_unavailable",
    ...
    try:
        if transport == "websocket":
            websocket_received_event = False
            try:
                async for event_block, event_type in _stream_responses_via_websocket(
                    ...
                    route_trace=route_trace,
                    enforce_openai_sdk_contract=enforce_openai_sdk_contract,
                    defer_pre_event_failure_health=transport_mode == "auto",
                ):
                    websocket_received_event = True
                ...
                async for event_block in _stream_via_http_after_websocket_failure(
                    rejection_status=exc.status,
                    rejection_message=str(exc),
                ):
                    yield event_block
            except CodexTransportError as exc:
                if not _should_fallback_to_http_after_websocket_transport_error(
                    transport_mode,
                    exc,
                    received_event=websocket_received_event,
                ):
                    raise
                async for event_block in _stream_via_http_after_websocket_failure(
                    ...
                    yield event_block
            except (StreamIdleTimeoutError, aiohttp.ClientConnectionError, asyncio.TimeoutError, TimeoutError) as exc:
                if not _should_fallback_to_http_after_websocket_transport_error(
                    transport_mode,
                    exc,
                    received_event=websocket_received_event,
    ...
    async def execute(self) -> CompactResponsePayload:
        ...
        try:
            if self.route is not None:
                ...
                if parsed:
                    # A successful HTTP/SSE status is not sufficient for the
                    # compact contract.  Codex requires exactly one opaque
                    # compaction item; upstream has occasionally returned a
                    # ``response.compaction`` envelope with an empty output
                    # array.  Treat that as an upstream parse failure so the
                    ...
                    if (
                        ...
                        failure_phase = "parse"
                        failure_detail = "compact_output_missing"
                        retryable_same_contract = False
                        raise ProxyResponseError(
                            502,
                            openai_error("compact_output_missing", error_message, error_type="server_error"),
                            failure_phase=failure_phase,
                            retryable_same_contract=retryable_same_contract,
                            failure_detail=failure_detail,
                            upstream_status_code=status_code,
            ...
            async with _service_circuit_breaker_context(
                ...
                if parsed:
                    # See the routed-client branch above: an empty
