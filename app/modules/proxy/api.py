...
def _to_v1_usage_limit_response(limit: ApiKeySelfLimitData) -> V1UsageLimitResponse:
    ...
    reset_at_text = reset_at.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
...
async def _build_codex_usage_payload_for_api_key(
    api_key: ApiKeyData,
    *,
    ...
    async with get_background_session() as session:
        ...
        usage = await service.get_key_usage_summary_for_self(api_key.id)
        reservation = (
            await repository.get_usage_reservation(exclude_reservation_id)
    ...
    reservation_deltas: dict[tuple[str, datetime], int] = {}
    if reservation is not None and reservation.status == "reserved" and reservation.api_key_id == api_key.id:
        for item in reservation.items:
            reset_at = item.expected_reset_at
            if reset_at.tzinfo is None:
                reset_at = reset_at.replace(tzinfo=timezone.utc)
            reservation_deltas[(item.limit_type.value, reset_at.astimezone(timezone.utc))] = item.reserved_delta
    ...
    for limit in usage.limits:
        ...
        if reset_at.tzinfo is None:
            reset_at = reset_at.replace(tzinfo=timezone.utc)
        reserved_delta = reservation_deltas.get(
            (limit.limit_type, reset_at.astimezone(timezone.utc)),
            0,
        )
        if reserved_delta > 0:
            limit = replace(
                limit,
                current_value=max(0, limit.current_value - reserved_delta),
                remaining_value=min(limit.max_value, limit.remaining_value + reserved_delta),
            )
        key_limits.append(_to_v1_usage_limit_response(limit))
...
async def _rate_limit_headers_with_reservation_cleanup(
    ...
    *,
    ...
    try:
        return await _rate_limit_headers_for_request(context, api_key, owned_reservation)
...
def _codex_rate_limit_headers_from_payload(payload: RateLimitStatusPayloadData) -> dict[str, str]:
    ...
    for label, window in (
        ...
        ("monthly", details.monthly_window),
    ):
        if window is None or window.limit_window_seconds is None:
            continue
        headers[f"x-codex-{label}-used-percent"] = str(float(window.used_percent))
        headers[f"x-codex-{label}-window-minutes"] = str(int(window.limit_window_seconds / 60))
        if window.reset_at is not None:
            headers[f"x-codex-{label}-reset-at"] = str(int(window.reset_at))
    credits = payload.credits
    if credits is not None:
        headers["x-codex-credits-has-credits"] = "true" if credits.has_credits else "false"
        headers["x-codex-credits-unlimited"] = "true" if credits.unlimited else "false"
        if credits.balance is not None:
            headers["x-codex-credits-balance"] = credits.balance
    return headers
...
async def _normalize_public_responses_stream(
    ...
    *,
    ...
    """
    terminal_seen = False
    native_client_retry_boundary = False
    done_seen = False
    contract_violation_kind: str | None = None
    ...
    async for event_block in stream:
        if event_block == NATIVE_CODEX_CLIENT_RETRY_BOUNDARY_FRAME:
            native_client_retry_boundary = not enforce_openai_sdk_contract
            continue
    ...
    if terminal_seen:
        if not done_seen and not enforce_openai_sdk_contract:
            yield "data: [DONE]\n\n"
        return
    if native_client_retry_boundary:
        return
