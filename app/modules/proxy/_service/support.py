...
# A transport-only failure should influence the next retry, but must not pin
# the whole continuity key away from an account forever.  This short window is
# long enough for the official client's bounded stream retry and expires on its
# own when the session is healthy again.
_WEBSOCKET_TRANSPORT_FAILURE_EXCLUSION_TTL_SECONDS = 120.0
...
class _WebSocketContinuityState:
    ...
    # Account-neutral transport evidence is retained only as a one-shot
    # exclusion for the next logical retry.  It is deliberately not account
...
class _WebSocketUpstreamControl:
    ...
    close_downstream_for_client_retry: bool = False
...
def _websocket_request_can_replay_before_visible_output(
    ...
    *,
    ...
    allow_account_exhaustion_replay: bool = False,
) -> bool:
