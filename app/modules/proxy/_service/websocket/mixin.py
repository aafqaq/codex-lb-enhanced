...
async def _process_and_forward_upstream_websocket_text(
    ...
    *,
    ...
    close_downstream_for_client_retry = upstream_control.close_downstream_for_client_retry
    ...
    upstream_control.close_downstream_for_client_retry = False
    ...
    if downstream_texts is not None:
        ...
        try:
            ...
            return True
    if close_downstream_for_client_retry:
        # Once any model output is visible, replaying inside the proxy can
        # duplicate text or tool side effects.  End the transport without a
        # terminal response event so the official Codex client retries the
        # complete sampling request using its own turn history, exactly as it
        # does for an interrupted official upstream stream.
        ...
        try:
            ...
            _facade().logger.debug(
                "Failed to close quota-exhausted upstream websocket before client retry",
                exc_info=True,
            )
        return True
...
async def _process_upstream_websocket_transport_end(
    ...
    *,
    ...
    # are still pre-visible: the replay helper below requires that no model
    # output/tool side effect has crossed the downstream boundary and consumes
    # at most one replay budget.  Once output is visible it refuses replay, so
    # account-neutral classification remains independent from retryability.
    # A frame-less close/error is transport evidence even when the adapter did
    ...
    if account_neutral and replay_request_state is None:
        ...
        # Keep the refusal reason explicit for the existing downstream close
        # contract and retain the no-penalty account settlement below.
        if any(state.last_downstream_sequence_number is not None for state in reader_owned):
            replay_refusal_reasons.append("account_neutral_transport_after_visible_output")
    if replay_request_state is not None:
        if account_neutral:
            # A pre-visible request is safe to replay, but the socket that
            # failed is still a poor candidate for the immediate retry.  Move
            # the retry off this sticky account without recording a health
            # penalty; a later turn may select it normally after the marker
            # expires/gets consumed.
...
class _WebSocketMixin:
        ...
        try:
            while True:
                ...
                if (
                    ...
                    # A full resend is portable across accounts; a short
                    # continuation whose context exists only on the failed
                    # account is not.  Keep the latter owner-pinned and let
                    # the existing fail-closed path explain the conflict.
                    if request_state.previous_response_id is None or request_state.fresh_upstream_request_is_retry_safe:
                ...
                if upstream is None:
                    ...
                    if (
                        ...
                        and account.id != transport_failure_account_id
                    ):
                        # The retry moved to a different account.  Consume the
                        # one-shot exclusion so later turns can use the pool
    ...
    async def _prepare_websocket_response_create_request(
        ...
        *,
        ...
        # A transport-only upstream failure is not account health evidence, but
        # the official Codex retry must not immediately select the same sticky
        # account that just lost its socket.  Apply the short-lived exclusion
        # to this logical continuity key.  If the request carries a proxy-
        # injected/client anchor and a verified full resend exists, install it
        # now so the replacement account does not need the old account's
        # response object.
        transport_failure_account_id = _websocket_transport_failure_account_id(continuity_state)
        if transport_failure_account_id is not None:
            request_state.excluded_account_ids.add(transport_failure_account_id)
            affinity_policy = replace(affinity_policy, reallocate_sticky=True)
            request_state.affinity_policy = affinity_policy
            if request_state.preferred_account_id == transport_failure_account_id or (
                request_state.replay_required_account_id == transport_failure_account_id
            ):
                if request_state.fresh_upstream_request_is_retry_safe and request_state.fresh_upstream_request_text:
                    fresh_replay_text = _install_verified_fresh_replay(
                        request_state,
                        require_proxy_injected_previous_response_id=False,
                        require_account_neutral=False,
                    )
                    if fresh_replay_text is not None:
                        text_data = fresh_replay_text
                request_state.preferred_account_id = None
                request_state.replay_required_account_id = None
    ...
    async def _select_websocket_connect_account(
        ...
        *,
        ...
        if (
            ...
            # Per-request exclusions can make the final selection look like a
            # generic empty pool.  At this point every selectable account has
            # already returned quota exhaustion, so expose the real pool-wide
            # terminal instead of the misleading ``no_accounts`` wrapper.
        ...
        if retry_error_code is not None:
            if retry_is_previous_response_not_found:
                ...
                if account_exhaustion_retry:
                    # A quota terminal can arrive either before response.created
                    # or immediately after a created-only frame.  Neither shape
                    # has exposed model output, so retain/rewrite the visible
                    # response id and walk to a different pool account.  The
                    ...
                    if retry_text is None:
                        ...
                        if _prepare_websocket_request_state_for_visible_output_replay(request_state) is None:
                            ...
                            await proxy._handle_stream_error(
                                account,
                                {"message": retry_error_message or "Upstream usage limit reached"},
                                retry_error_code,
                            )
                            _facade().logger.info(
                                "Retrying quota-exhausted websocket request on another account "
                                "request_id=%s exhausted_account_id=%s excluded_count=%s",
                                request_state.request_log_id or request_state.request_id,
                                account.id,
                                len(request_state.excluded_account_ids),
                            )
                else:
                    upstream_control.reconnect_requested = True
                    request_state.replay_count += 1
    ...
    async def _finalize_websocket_request_state(
        ...
        *,
        ...
        error_code = upstream_usage_limit_error_code(error_code, error_message) or error_code
        ...
        if settlement.account_health_error:
            ...
            if not settlement_committed:
                ...
                try:
                    ...
                    if not isolate_account_health_failure:
                        raise
        # Persist the response owner even when the turn ended abnormally.  A
        # later client reconnect may arrive hours or days after this socket was
        # gone; relying only on the in-memory websocket index then loses the
        # account pin and makes ``previous_response_id`` look orphaned.  The
        # request-log handoff above is the durable source of truth, while this
        # cache keeps the hot path cheap for the next turn.
