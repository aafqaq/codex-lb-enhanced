...
from app.modules.proxy._service.support import (
    ...
    _WEBSOCKET_TRANSPORT_FAILURE_EXCLUSION_TTL_SECONDS,
...
def _record_websocket_transport_failure(
    continuity_state: _WebSocketContinuityState | None,
    *,
    account_id: str | None,
) -> None:
    """Remember one neutral transport failure for the next client retry.

    This is the missing bridge between Codex's retry loop and a sticky
    account pool: the next full request must not immediately land on the same
    account, yet the failure must not be written as account health evidence.
    The marker is intentionally tiny, in-memory, and expiring.
    """

    if continuity_state is None or not isinstance(account_id, str) or not account_id.strip():
        return
    continuity_state.transport_failure_account_id = account_id.strip()
...
def _websocket_transport_failure_account_id(
    ...
    if continuity_state is None:
        return None
    account_id = continuity_state.transport_failure_account_id
    failed_at = continuity_state.transport_failure_at
    if account_id is None or failed_at is None:
        return None
    if time.monotonic() - failed_at >= _WEBSOCKET_TRANSPORT_FAILURE_EXCLUSION_TTL_SECONDS:
    ...
    *,
    ...
    if account_exhaustion_code is not None:
        ...
        if request_state.previous_response_id is not None and request_state.preferred_account_id is not None:
            # Let the caller's owner branch either install a proof-gated full
            # replay or fail closed with previous_response_owner_unavailable.
            # Returning None here would leak the raw quota event from one
            # continuity owner to the client.
            return account_exhaustion_code
        # A quota frame is emitted by the upstream only after it accepted this
        # response.create attempt.  The sender timestamp can be absent during
        # the pre-created race, so do not gate account-pool walking on the
        # generic unsent-request predicate (which is intentionally stricter for
