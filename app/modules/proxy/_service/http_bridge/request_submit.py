    ...
    async def _submit_http_bridge_request_with_handoff(
        ...
        *,
        ...
        try:
            ...
            # Every send_text exception is transport-side evidence. Delivery
            # may be ambiguous, so this path remains terminal and never
            # reconnects-and-resends, but it must not mutate account health.
            # Structured upstream application failures arrive on the reader
            # path and retain their existing account-health classification.
        ...
        *,
        ...
        def request_is_retryable(request_state: _WebSocketRequestState) -> bool:
            if _websocket_request_can_replay_before_visible_output(
                request_state,
                allow_account_exhaustion_replay=allow_account_exhaustion_failover,
            ):
                return True
            if (
            ...
            async with session.pending_lock:
                ...
                if len(retryable_candidates) == 1:
                    ...
                    fresh_hard_request_account_switch_candidate = (
                        ...
                        and (allow_account_exhaustion_failover or candidate.replay_count == 0)
                    )
        ...
        async with session.pending_lock:
            ...
            if (
                ...
                and not allow_account_exhaustion_failover
            ):
                return False
        ...
        try:
            ...
            if quota_hard_account_switch:
                ...
                await self._reconnect_http_bridge_session(
                    session,
                    request_state=request_state,
                    # Quota failover has already been handled above with an
                    # explicit reallocation policy.  Ordinary retries retain
                    # the session's existing affinity and reconnect options.
                    **reconnect_reader_kwargs,
                )
