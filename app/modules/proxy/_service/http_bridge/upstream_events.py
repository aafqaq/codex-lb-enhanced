...
# upstream may have accepted the request while the transport was silent. Track
# repeated failures per continuity key so a poisoned anchor can be cleared
# without converting anonymous transport evidence into account-health writes.
...
async def _record_http_bridge_account_timeout_signal(
    ...
    *,
    ...
    """Clear poisoned continuity after repeated eventless transport failures.

    This is deliberately separate from account health. A timeout or abrupt
    eventless transport drop cannot be replayed safely for a continuity-bound
    turn, but repeated failures of the same bridge key are evidence against
    that key's durable anchor rather than every account that may serve it.
    """

    account_id = session.account.id
    signal_key = getattr(session, "key", account_id)
    now = time.monotonic()
    async with service._http_bridge_account_timeout_lock:
        failures = service._http_bridge_account_timeout_failures.setdefault(signal_key, [])
        failures[:] = [
        ...
        if len(failures) < _HTTP_BRIDGE_ACCOUNT_TIMEOUT_EJECTION_THRESHOLD:
            return
        # Start a fresh evidence window after one fenced reset attempt. A
        # failed clear remains fail-closed and must not fall back to an account
        # penalty merely because the durable owner changed concurrently.
        failures.clear()

    poison_detail = _http_bridge_anchor_poison_detail(detail) or f"repeated_{detail}"
    cleared = await _abandon_durable_http_bridge_continuity(service, session, detail=poison_detail)
    if cleared:
        logger.warning(
            "HTTP bridge continuity reset after repeated eventless upstream failures "
        ...
        logger.info(
            "HTTP bridge continuity reset unavailable after repeated eventless upstream failures "
            "account_id=%s detail=%s",
            account_id,
            poison_detail,
        )
    ...
    async def _relay_http_bridge_upstream_messages(
        ...
        try:
            while True:
                ...
                # An abrupt drop with no peer close frame is transport evidence,
                # not account evidence. Output makes replay unsafe but does not
                # change that provenance. Repeated eventless drops still feed
                # the continuity-isolation signal inside the failure path.
    ...
    async def _process_parsed_http_bridge_upstream_event(
        ...
        *,
        ...
        account_exhaustion_after_visible_output = bool(
            ...
            and (
                status_request_state.downstream_visible
                or status_request_state.upstream_model_output_seen
        ...
        if account_exhaustion_after_visible_output and status_request_state is not None:
            ...
            # the downstream SSE without response.completed; the official
            # Codex client treats that as a retryable sampling interruption
            # and rebuilds the turn from its local history.  The exhausted
            # account is persisted before the replacement request arrives.
        ...
        if (
            ...
            if status_request_state is not None and status_request_state.previous_response_id is None:
                ...
                retried = await self._retry_http_bridge_precreated_request(
                    session,
                    allow_account_exhaustion_failover=account_exhaustion_retry,
                )
                if retried:
                    return
                async with session.pending_lock:
        ...
        if terminal_event_queue is not None:
            if account_exhaustion_after_visible_output:
                await terminal_event_queue.put(NATIVE_CODEX_CLIENT_RETRY_BOUNDARY_FRAME)
