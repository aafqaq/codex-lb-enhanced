        ...
        *,
        ...
        try:
            ...
            if event and event.type in ("response.failed", "error"):
                ...
                code = upstream_usage_limit_error_code(code, raw_error_message) or code
            ...
            async for line in iterator:
                ...
                if event:
                    if event_type in ("response.failed", "error"):
                        ...
                        raw_error_code = (
                            upstream_usage_limit_error_code(raw_error_code, raw_error_message) or raw_error_code
                        )
                        rewritten_error = _facade()._rewrite_previous_response_stream_error(
                            previous_response_id=payload.previous_response_id,
                            preferred_account_id=preferred_account_id,
                        ...
                        if rewritten_error is not None:
                            ...
                            if account_exhaustion_after_visible_output:
                                # The official Codex client retries a sampling
                                # request when a stream ends without a terminal
                                # response event.  After output is visible, that
                                # client-owned retry boundary is safer than
                                # replaying inside the proxy (which could
                                # duplicate text or tool side effects).  Mark
                                # this account exhausted, hide its terminal
                                # quota event, and let the retry select another
                                # pool account.
