...
def _is_account_neutral_transport_drop(
    ...
    *,
    ...
    An abrupt transport drop that carries no close frame is not evidence of
    account ill-health. Response events make replay unsafe, but they do not
    prove whether the account, an upstream proxy, or the local network ended
    the physical connection. Charging the account lets one broken continuity
    path drain otherwise usable pool accounts. Any peer-authored close frame —
    even a non-clean one — keeps the existing penalty semantics.
