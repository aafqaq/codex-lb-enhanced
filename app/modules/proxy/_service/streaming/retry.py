...
class _StreamingRetryMixin:
    async def _stream_with_retry(
        ...
        *,
        ...
        account_exhaustion_walk_active = False
        ...
        try:
            ...
            for attempt in count():
                if attempt >= max_attempts and not account_exhaustion_walk_active:
                    break
                ...
                try:
                    ...
                    if effective_attempt_timeout <= 0:
                        ...
                        return
                    transient_retries = 0
                    allow_retry_flag = attempt < max_attempts - 1 or account_exhaustion_walk_active
                    while True:
                        stream_timeout_tokens = _facade()._push_stream_attempt_timeout_overrides(
                            _facade()._remaining_budget_seconds(deadline),
                        )
                        try:
                            ...
                            if settlement.downstream_visible:
                                ...
                                if account_exhaustion_code is not None:
                                    ...
                                    # client resubmits the whole sampling
                                    # request. Public OpenAI-compatible routes
                                    # retain their explicit terminal failure.
                                    settlement.record_success = False
                                    settlement.error_code = account_exhaustion_code
                                    settlement.error_message = error_message
                                    if isinstance(tex, ProxyResponseError):
                                    ...
                                    if settled:
                                        await proxy._handle_stream_error(
                                            account,
                                            _stream_settlement_error_payload(settlement),
                                            account_exhaustion_code,
                                        )
                                    if not enforce_openai_sdk_contract:
                                        _facade().logger.info(
                                            "Ending visible quota-exhausted stream for native client retry "
                                            "request_id=%s account_id=%s",
                                            request_id,
                                            account.id,
                                        )
                                        yield NATIVE_CODEX_CLIENT_RETRY_BOUNDARY_FRAME
                                    else:
                                        yield format_sse_event(event)
                                    return
                                if isinstance(tex, ProxyResponseError):
                    ...
                    if account_exhaustion_code_for_failover(exc.code, exc.error.get("message")) is not None:
                        ...
                        # partial-pool limit to the client.
                        account_exhaustion_walk_active = True
                    else:
                        # The extended budget is only for the finite quota walk.
                        # A later, unrelated failure returns to the ordinary
                        # retry bound instead of inheriting an unbounded loop.
                        account_exhaustion_walk_active = False
