...
_TRANSIENT_CODES = frozenset(
    {
        ...
        # A 2xx compact envelope with no opaque item is an upstream
        # protocol/transient failure.  Classify it as retryable so compact
        # service can exclude the bad account and walk the pool.
        "compact_output_missing",
    }
)
_MODEL_CAPACITY_MESSAGE_MARKERS = ("selected model is at capacity",)
_UPSTREAM_USAGE_LIMIT_CODES = frozenset(
    {
        "rate_limit_exceeded",
        "usage_limit_reached",
        "insufficient_quota",
...
# Internal SSE control frame.  It is consumed by the API stream normalizer and
# never forwarded to clients.  A quota failure after visible output cannot be
# replayed safely inside the proxy (text or tool calls may already have caused
# side effects), so the service uses this marker to ask the outer layer to end
# the response without synthesizing response.failed.  Codex then performs its
# native whole-sampling retry with the exhausted account already excluded.
NATIVE_CODEX_CLIENT_RETRY_BOUNDARY_FRAME = ": codex-lb native-client-retry-boundary\n\n"
_UPSTREAM_USAGE_LIMIT_MESSAGE_MARKERS = (
...
def upstream_usage_limit_error_code(code: str | None, message: str | None) -> str | None:
    """Return the account-exhaustion code even for message-only upstream errors.

    ChatGPT's Codex backend has emitted the same account quota condition with
    different outer error types over time.  In particular, some websocket
    responses carry ``invalid_request_error`` (or no code) while preserving
    only ``The usage limit has been reached`` in the message.  Treating that as
    an ordinary request error leaks a single account's exhaustion to the
    client instead of letting the pool select another account.
    """

    normalized_code = _normalize_error_code(code, None)
    if normalized_code in _UPSTREAM_USAGE_LIMIT_CODES:
