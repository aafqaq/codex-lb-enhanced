...
class _EmptyCompactionResponse:
    """Upstream's intermittent 200 response with no compaction item."""

    status_code = 200
    headers = {"content-type": "application/json"}

    def json(self) -> dict[str, object]:
        return {"object": "response.compaction", "id": "resp_empty", "output": []}
...
@pytest.mark.asyncio
async def test_compact_responses_rejects_empty_compaction_success_as_retryable_upstream_error(
    route: ResolvedUpstreamRoute,
) -> None:
    client = _RouteMetadataCodexClient(_EmptyCompactionResponse())
    payload = ResponsesCompactRequest(model="gpt-5.2", instructions="Summarize.", input="hello")
    ...
    assert exc.upstream_error_code == "compact_output_missing"
