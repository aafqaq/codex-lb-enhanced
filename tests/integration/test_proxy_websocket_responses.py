...
def test_backend_responses_websocket_transparently_retries_precreated_usage_limit_reached(app_instance, monkeypatch):
        ...
        [
            _FakeUpstreamMessage(
                ...
                text=json.dumps(
                    {
                        "type": "response.created",
                        "response": {"id": "resp_ws_quota_created", "status": "in_progress"},
                    },
                    separators=(",", ":"),
                ),
            ),
                ...
                text=json.dumps(
                    {
                        "type": "response.failed",
                        "response": {
                            "id": "resp_ws_quota_created",
                            "status": "failed",
                            # The live backend has also used a generic outer
                            # code while retaining only this message.  It is
                            # still account exhaustion, not a client error.
                            "error": {
    ...
    third_upstream = _FakeUpstreamWebSocket(
        [
            _FakeUpstreamMessage(
                "text",
                text=json.dumps(
    ...
    upstreams = [first_upstream, second_upstream, third_upstream]
    connect_models: list[str | None] = []
    handled_error_codes: list[str] = []
    ...
    assert second_event["type"] == "response.completed"
    # Both exhausted accounts are hidden from the client.  The created frame
    # from account two is retained as the public response id while account
    # three's replay-created frame is suppressed and its completion rewritten.
    assert first_event["response"]["id"] == "resp_ws_quota_created"
    assert second_event["response"]["id"] == "resp_ws_quota_created"
    assert connect_models == ["gpt-5.1", "gpt-5.1", "gpt-5.1"]
    assert handled_error_codes == ["usage_limit_reached", "usage_limit_reached"]
...
def test_backend_responses_websocket_visible_usage_limit_uses_native_client_retry_boundary(
                ...
                text=json.dumps(
                    {
                        "type": "response.created",
                        "sequence_number": 0,
                        "response": {"id": "resp_ws_visible_quota", "status": "in_progress"},
                    },
                    separators=(",", ":"),
                ),
            ),
                ...
                text=json.dumps(
                    {
                        ...
                        "sequence_number": 1,
                        "response_id": "resp_ws_visible_quota",
                        "item_id": "msg_ws_visible_quota",
                        "output_index": 0,
                        "content_index": 0,
                        "delta": "partial",
                    },
        ...
        [
            _FakeUpstreamMessage(
                ...
                text=json.dumps(
                    {
                        "type": "response.created",
                        "response": {"id": "resp_ws_visible_retry_ok", "status": "in_progress"},
                    },
                    separators=(",", ":"),
                ),
            ),
    ...
    async def fake_connect_proxy_websocket(self, headers, **kwargs):
        ...
        connect_count += 1
        return SimpleNamespace(id=f"acct_ws_visible_{connect_count}", status=AccountStatus.ACTIVE), upstream
    ...
    request_payload = {
        ...
        "model": "gpt-5.6-sol",
        "instructions": "",
        "input": [{"role": "user", "content": [{"type": "input_text", "text": "continue safely"}]}],
        "stream": True,
    }
    ...
    with TestClient(app_instance) as client:
        ...
        # The official client opens a fresh transport and reissues the sampling
        # request.  This second logical attempt is routed to the next account.
        with client.websocket_connect("/backend-api/codex/responses") as retry_websocket:
            retry_websocket.send_text(json.dumps(request_payload))
            retry_created = json.loads(retry_websocket.receive_text())
            retry_completed = json.loads(retry_websocket.receive_text())

    assert created["type"] == "response.created"
    assert delta["type"] == "response.output_text.delta"
    assert delta["delta"] == "partial"
    assert disconnect.value.code == 1011
    assert retry_created["type"] == "response.created"
    assert retry_completed["type"] == "response.completed"
    assert connect_count == 2
