...
@pytest.mark.asyncio
async def test_codex_usage_api_key_maps_token_and_cost_limits_to_codex_windows(async_client, db_setup):
    key_id, plain_key = await _create_api_key(
        name="codex-usage-api-key-non-credit",
        limits=[
            LimitRuleInput(limit_type="total_tokens", limit_window="daily", max_value=2000),
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["rate_limit"]["primary_window"]["used_percent"] == 25
