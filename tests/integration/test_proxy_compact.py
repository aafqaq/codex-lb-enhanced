...
@pytest.mark.asyncio
async def test_proxy_compact_api_key_custom_limits_replace_pool_quota_headers(async_client, monkeypatch):
    email = "compact-key-quota@example.com"
    raw_account_id = "acc_compact_key_quota"
    auth_json = _make_auth_json(raw_account_id, email)
    response = await async_client.post(
        "/api/accounts/import",
        files={"auth_json": ("auth.json", json.dumps(auth_json), "application/json")},
    ...
    key_id, key = await _create_api_key(
        name="compact-key-own-quota",
        assigned_account_ids=[expected_account_id],
        limits=[
            LimitRuleInput(limit_type="total_tokens", limit_window="daily", max_value=1000),
            LimitRuleInput(limit_type="cost_usd", limit_window="weekly", max_value=1000),
        ],
    )
    async with SessionLocal() as session:
        await ApiKeysRepository(session).replace_limits(
            key_id,
            [
                ApiKeyLimit(
                    api_key_id=key_id,
    ...
    settings = await async_client.put(
        "/api/settings",
        json={"apiKeyAuthEnabled": True, "hideUpstreamQuotaFromApiKeys": False},
    )
    ...
    assert response.status_code == 200
    assert response.headers["x-codex-primary-used-percent"] == "25.0"
    assert response.headers["x-codex-primary-window-minutes"] == "1440"
    assert response.headers["x-codex-secondary-used-percent"] == "12.0"
    assert response.headers["x-codex-secondary-window-minutes"] == "10080"
    assert response.headers.get("x-codex-credits-balance") is None
    assert response.headers["x-codex-primary-used-percent"] != "91.0"
