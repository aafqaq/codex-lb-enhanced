...
async def test_stream_message_only_usage_limit_walks_entire_pool_past_generic_attempt_budget(
    ...
    for suffix in ("a", "b", "c", "d"):
        await _import_account(
            async_client,
            f"acc_stream_quota_walk_{suffix}",
            f"stream-quota-walk-{suffix}@example.com",
        )
    ...
    async def fake_stream(payload, headers, access_token, account_id, base_url=None, raise_for_status=False):
        ...
        if len(seen_account_ids) < 4:
            ...
            return
        yield _success_sse_event("resp_stream_quota_walk_ok")
    ...
    assert len(set(seen_account_ids)) == 4
...
@pytest.mark.asyncio
async def test_stream_rate_limit_walks_all_accounts_before_returning_actual_error(async_client, monkeypatch):
    """The generic three-attempt budget must not truncate a quota pool walk."""
    ...
    await _import_account(async_client, "acc_rl_d", "rld@example.com")
...
@pytest.mark.asyncio
async def test_stream_mid_stream_quota_uses_native_client_retry(async_client, monkeypatch):
    """A quota terminal after lifecycle output uses the native Codex retry boundary."""
    ...
    async with async_client.stream("POST", "/backend-api/codex/responses", json=payload) as retry_resp:
...
@pytest.mark.asyncio
async def test_stream_http_429_after_text_ends_for_native_client_retry_on_next_account(async_client, monkeypatch):
    """Visible output uses Codex's whole-request retry boundary instead of unsafe proxy replay."""
    ...
    # This is the request the official client issues after seeing a stream
    # without response.completed.  The exhausted account must already be out
    # of selection, and no single-account quota event may have leaked.
    async with async_client.stream("POST", "/backend-api/codex/responses", json=payload) as retry_resp:
        assert retry_resp.status_code == 200
        retry_lines = [line async for line in retry_resp.aiter_lines() if line]
